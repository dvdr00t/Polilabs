...
typedef struct thread_struct_s {
    float *array1;  // v1
    float *array2;  // column of mat
    float *array3;  // if last thread, v2, else NULL
    int size;
    float res;
    bool is_valid;
    int thread_ID;
} thread_struct_t;
thread_struct_t *threads_DATA;

/**
 * @brief Sempahore used in thread's routine function
 * 
 */
static sem_t sem;
...
static void* routine (void *data) {
    ...
    // fprintf(stdout, "[DEBUG] I am thread %d...\n", thread_DATA->thread_ID);
    ...
    if (thread_DATA->thread_ID == thread_DATA->size-1) {
        for (int i = 0; i < thread_DATA->size-1; i++) {
            sem_wait(&sem); // waiting all other threads before proceeding
        }   

        /* STORING RESULT */
        float *tmp = (float *) malloc(thread_DATA->size * sizeof(float));
        if (tmp == NULL) {
            fprintf(stderr, "[ERROR] malloc() failed execution\n");
            thread_DATA->is_valid = false;
            pthread_exit((void *) EXIT_FAILURE);
        ...
        for (int i = 0; i < thread_DATA->size; i++) {
            tmp[i] = threads_DATA[i].res;
        }

        /* COMPUTING RESULT */
        int err = array_mul(tmp, thread_DATA->array3, thread_DATA->size, &thread_DATA->res);
        if (err) {
            fprintf(stderr, "[ERROR] array_mul() failed execution\n");
            thread_DATA->is_valid = false;
            pthread_exit((void *) EXIT_FAILURE);
...
int solver(int N) {
    ...
    threads_DATA = (thread_struct_t *) malloc(N * sizeof(thread_struct_t));
    if (threads_ID == NULL || threads_DATA == NULL) {
        fprintf(stderr, "[ERROR] malloc() failed execution\n");
        return -1;
    }

    /* BARRIES INITIALIZATION */
    sem_init(&sem, 0, 0);
    ...
    for (int id = 0; id < N; id++) {
        ...
        threads_DATA[id].array3 = (id == N-1) ? v2 : NULL;
    ...
    /* FINAL RESULT */
    float res = threads_DATA[N-1].res;
