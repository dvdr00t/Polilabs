...
; 	- bit[16] -> read-only bit. Read as 1 if counter reaches 0 since last time this register is read; 
;				 clear to 0 when read or when current counter value is cleared.
; 	- bit[2] -> read/write bit. It stores 1 in order to use processor free running clock. It stores 0 in order 
;				to use external reference clock (STCLK).
; 	- bit[1] -> read/write bit. It stores 1 in order to generate interrupt when timer reaches 0. It stores 0 
;				in order to prevent generation of interrupts.
; 	- bit[0] -> read/write bit. It stores 1 in order to enable SysTick Timer. It stores 0 in order to disable SysTick Timer.
ControlAndStatusRegister EQU 0xE000E010
	
; The Reload Value Register is a 24-bits register (R/W) specified 
; in the Nested Interrupt Vectored Controller (NVIC) at the address 0xE000E014. It stores
; the initial value of the timer (i.e. the fixed interval at which the clock raises the exception).
ReloadValueRegister EQU 0xE000E014
	
; The Current Value Register is a 24-bits register (R/W) specified 
; in the Nested Interrupt Vectored Controller (NVIC) at the address 0xE000E018. It stores
...
; The handler manages the iteration_counter of the loop
