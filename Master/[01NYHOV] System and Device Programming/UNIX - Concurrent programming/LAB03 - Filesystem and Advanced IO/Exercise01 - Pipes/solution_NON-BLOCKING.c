...
int set_fnctl(int file_descriptor, int flags);
int clr_fnctl(int file_descriptor, int flags);
...
int parent_routine(int pipe_1, int pipe_2, int STR_NUM, int STR_SIZE) {
    ...
    /* SETTING PIPES AS NON_BLOCKING */
    if (set_fnctl(pipe_1, O_NONBLOCK) != 0 || set_fnctl(pipe_2, O_NONBLOCK) != 0) {
        fprintf(stderr, "[ERROR] set_fcntl() failed execution\n");
        return -1;
    }

    /* INITIATE NON-BLOCKING POLLING OPERATION */
    int count_messages = 0;
    bool running = true;
    while (running) {

        /* POLLING FROM PIPE 1 */
        if (read(pipe_1, header, HEADER_SIZE) <= 0) {
            fprintf(stdout, "[STATUS] ... nothing in pipe_1 ...\n");
        } else {
        ...
        /* POLLING FROM PIPE 2 */
        if (read(pipe_2, header, HEADER_SIZE) <= 0) {
            fprintf(stdout, "[STATUS] ... nothing in pipe_2 ...\n");
        } else {
    ...
    /* REMOVE FROM PIPES NON_BLOCKING FLAG */
    if (clr_fnctl(pipe_1, O_NONBLOCK) != 0 || clr_fnctl(pipe_2, O_NONBLOCK) != 0) {
        fprintf(stderr, "[ERROR] clr_fcntl() failed execution\n");
        return -1;
    }
...
int set_fnctl(int file_descriptor, int flags) {
    ...
    /* GETTING SET OF FLAGS OF THE FILE PASSED AS ARGUMENT */
    int set_of_flags;
    if ((set_of_flags = fcntl(file_descriptor, F_GETFL, 0)) < 0) {
        fprintf(stderr, "[ERROR] fctnl() failed execution or set of flags is null\n");
        return -1;
    }
    ...
    /* SETTING THE NEW SET OF FLAGS */
    if (fcntl(file_descriptor, F_SETFL, set_of_flags) < 0) {
        fprintf(stderr, "[ERROR] fctnl() failed execution\n");
        return -1;
    }
