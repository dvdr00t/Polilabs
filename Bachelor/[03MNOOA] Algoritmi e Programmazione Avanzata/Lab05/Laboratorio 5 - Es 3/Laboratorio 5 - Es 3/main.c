...
void linear_search(char *station_name, dato *transport, int rows);             //Linear search
void binary_search(char *station_name, dato *transport, int rows);         //Binary search
...
void reading_file(FILE *fp, dato **transport, int *rows);              //Reading data from file and saving it in a struct
...
int main(int argc, const char * argv[]){
    ...
    reading_file(fp, &transports, &rows);   //Reading the file and store data
    ...
    while (flag == 0){
        ...
        switch (command){
            ...
            case 7:
                ...
                //Deallocating space of used array
                free(transports);
                for (i = 0; i < 4; i++)
                    transports_sorted[i] = NULL;
                
                //Reading the file store data
                reading_file(fp, &transports, &rows);
                fclose(fp);
...
dato* merge_sort(dato *transport_A, dato *transport_B, int rows, int operation){
    ...
    //dynamic allocation for transport_B
...
void print_menu(void){
    ...
    printf("7) Read a new file of data.\n");
...
void reading_file(FILE *fp, dato **transport, int *rows){       //Dynamic allocation of memory and saving data from file in an array of struct
    int i = 0;
    char line[MAX_LINE];
    ...
    (*transport) = (dato*) malloc((*rows) * sizeof(dato));
    if ((*transport) == NULL){
    ...
    while (fgets(line, MAX_LINE, fp) != NULL){
        sscanf(line, "%s %s %s %s %s %s %d", (*transport)[i].code, (*transport)[i].start, (*transport)[i].end, (*transport)[i].date, (*transport)[i].time_start, (*transport)[i].time_end, &(*transport)[i].delay);
        ++i;
    }
}
