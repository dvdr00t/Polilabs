...
#define INTEGER_SIZE 8                      /* 8 bytes = 64 bits */
...
int main (int args, char *argv[]) {
    ...
    unsigned char A_hex[INTEGER_SIZE];       /* 64 bits integer */
    unsigned char B_hex[INTEGER_SIZE];       /* 64 bits integer */
    ...
    /* computing result */
    unsigned long long int mod = ((unsigned long) 1 << INTEGER_SIZE);
    C_int = (unsigned long long) (A_int + B_int) % mod;
    fprintf(stdout, "Printing C (int): %lld\n", C_int);

    /**
     * This should be repeated 4 times, since each integer can be represented with 64 bits but we need
     * 256 bits integers. 
    */
